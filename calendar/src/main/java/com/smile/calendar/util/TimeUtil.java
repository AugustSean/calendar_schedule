...
public class TimeUtil {
    ...
    public static String StrToDate(long str) {

        SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss", Locale.getDefault());
        String date = "";
        try {
            date = format.format(new Date(str));
        } catch (Exception e) {
            e.printStackTrace();
        }
        return date;
...
//    public static Date getDateFromTime(String dateTime) {
//        //设定时间的模板
//        SimpleDateFormat sDateFormat = new SimpleDateFormat("yyyy-MM-dd", Locale.getDefault());
...
//    }
    ...
    public static String getTimeFromDate(Date dateTime) {
        SimpleDateFormat sDateFormat = new SimpleDateFormat("yyyy-MM-dd", Locale.getDefault());
    ...
    public static Date timeFormatYYYYMMDD(String time) {
        try {
            // 将给定的字符串中的日期提取出来
    ...
    public static boolean compare_current(String DATE1) throws ParseException {
        DateFormat df = new SimpleDateFormat("yyyy-MM-dd hh:mm");
        String DATE2 = TimeUtil.StrToDate(System.currentTimeMillis());
        Date dt1 = df.parse(DATE1);
        Date dt2 = df.parse(DATE2);
        return (dt1.getTime() > dt2.getTime());
    }
